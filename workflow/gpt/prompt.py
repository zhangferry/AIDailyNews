...
3. 顺序总结内容：顺序处理每篇内容，以中文概括并归纳，总结内容在 200 个字符左右，或至少3个句子，必须让读者能够完整、准确了解文章内容，总结为中文内容。如果原始内容无法按以上要求总结，可以输出`省略`来表示略过。
4. 内容打分：评价原始内容，给出一个推荐值。促销、低价、影视、优惠、应用商店、app推荐内容，降低分值，技术、干货内容，增加分值，总分为10分，6分及以下表示不太推荐。
5. 最佳文章信息提取：给每篇内容取一个有吸引力的标题，标题以一个概括性的emoji开始，标题是中文；提取出概括内容的tags，tag 数量为1个
